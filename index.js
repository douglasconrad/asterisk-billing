	...
	function(x){
		...
		if(x.event === 'Newchannel'){
			...
			if(bill[x.linkedid]){
				...
				log.info("[%s] New channel in a already monitored call: %s", count[x.linkedid].count, JSON.stringify(bill[x.linkedid]));
			...
			try {
				if(bill[x.uniqueid]){
					if(x.channelstate === '6' && x.uniqueid !== x.linkedid){
						...
						log.info('Call was ANSWERED by the caller: %s', JSON.stringify(x));
					}
				}else if(bill[x.linkedid] && x.calleridnum !== x.connectedlinenum){
		...
		if(x.event === 'Hangup'){
				...
				if(bill[x.uniqueid]){
					...
					log.info('DEBUG: call [%s][%s] Hanguping the caller channel on the bridge: %s',count[x.uniqueid].count, x.uniqueid, JSON.stringify(bill[x.uniqueid]));
					if(count[x.uniqueid].count === 0){
						log.info('DEBUG: Saving billing on main call');
					...
					log.info('DEBUG: call [%s][%s] Hanguping the called channel on the bridge: %s',count[x.linkedid].count, x.uniqueid, JSON.stringify(bill[x.linkedid]));
					if(count[x.linkedid].count === 0){
						log.info('DEBUG: Saving billing on slave call');
