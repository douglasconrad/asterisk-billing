...
function insert(bill){
		...
		for (var i = bill.callflow.length - 1; i >= 0; i--) {
			...
			connection.query('INSERT INTO ' + config.mysql.tables.callflow + ' SET ?', callflow, function(err,result){
				if (err) {
					//connection.end();
					log.info(err);
				}
...
function search(data){
		...
		return q.Promise(function(resolve, reject){
			if(data){
				connection.query("SELECT * FROM ?", config.mysql.tables.cdr, function(err, rows, fields){
					if(err){
